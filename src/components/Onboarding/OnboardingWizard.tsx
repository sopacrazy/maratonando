...
const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ onComplete }) => {
  ...
  const [bio, setBio] = useState(
    user?.bio === "Novo na comunidade maratonando!" ? "" : user?.bio || ""
  );
  ...
  const saveData = async () => {
    ...
    try {
      ...
      // 4. Follow Users
      const followPromises = Array.from(followingIds).map((targetId: string) =>
      ...
      try {
        ...
        let welcomeStamp = await BadgeService.awardBadgeByName(
          user.id,
          "maratonando"
        );
        ...
        if (!welcomeStamp) {
          console.log(
            'Badge "maratonando" não encontrado, tentando "Maratonei"...'
          );
          welcomeStamp = await BadgeService.awardBadgeByName(
            user.id,
            "Maratonei"
          );
        }
        ...
        if (!welcomeStamp) {
          console.warn(
            '⚠️ Nenhum selo de boas-vindas ("maratonando" ou "Maratonei") foi encontrado no banco.'
          );
        }
      ...
      alert(
        "Houve um erro ao salvar seus dados. Por favor, verifique sua conexão e tente novamente no passo final."
      );
    } finally {
      setLoading(false);
    }
  ...
  const renderStepContent = () => {
    switch (currentStep) {
        ...
        return (
          <div className="text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-lg">
              O Maratonando vai muito além de uma lista. É a arena definitiva
              para fãs de séries: debata teorias, vença duelos de opinião e
              domine o ranking do seu clube
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 text-left mt-8">
              <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Desbloqueie selos de raridade{" "}
                  <span className="text-gray-400 font-bold">Comum</span>,{" "}
                  <span className="text-blue-400 font-bold">Raro</span>,{" "}
                  <span className="text-purple-400 font-bold">Épico</span> e{" "}
                  <span className="text-yellow-400 font-bold">Lendário</span>!
                </p>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Suas reviews têm poder. Construa sua reputação, conquiste
                  seguidores leais e veja suas opiniões moldarem o debate da
                  comunidade.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
            <div className="flex flex-col items-center gap-4">
              <div className="relative group cursor-pointer">
                <div
                  className="size-32 rounded-full bg-cover bg-center border-4 border-white dark:border-[#1a1122] shadow-xl"
      ...
      case 2: // Series
        return (
          <div className="space-y-4 flex flex-col animate-in fade-in slide-in-from-right-8 duration-300">
            <div>
            ...
            <div className="overflow-y-auto min-h-[200px] max-h-[50vh] pr-2">
              ...
              {selectedSeries.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {selectedSeries.map((s) => (
                    <div
                      key={s.id}
                      className="bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-bold flex items-center gap-2"
                    >
                      {s.name}
                      <button
                ...
                {searchResults.map((series) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative aspect-[2/3] rounded-lg overflow-hidden cursor-pointer group transition-transform hover:scale-105 ${
        ...
        return (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-8 duration-300">
            <p className="text-gray-600 dark:text-gray-300 text-center mb-4 text-sm">
              Siga alguns perfis para ver o que eles estão assistindo.
              (Opcional)
            </p>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      style={{ backgroundImage: `url('${sUser.avatar}')` }}
                    ></div>
                    <div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white dark:bg-[#1a1122] flex flex-col overflow-hidden">
      <div className="w-full max-w-2xl mx-auto flex flex-col h-full overflow-y-auto pb-safe">
        <div className="flex-shrink-0 p-4 pt-6">
          {/* Progress Bar */}
          <div className="w-full h-1.5 bg-gray-100 dark:bg-white/10 rounded-full mb-6 overflow-hidden">
            <div
        ...
        <div className="flex-shrink-0 flex items-center justify-between p-4 pt-4 pb-6 border-t border-gray-100 dark:border-white/5 bg-white dark:bg-[#1a1122]">
          <button
            onClick={() => setCurrentStep((prev) => Math.max(0, prev - 1))}
            className={`text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 font-bold px-4 py-2 min-h-[44px] touch-manipulation ${
          ...
          <button
            onClick={handleNext}
            disabled={
              loading || (currentStep === 2 && selectedSeries.length < 3)
            }
            className="bg-primary hover:bg-primary/90 text-white px-6 md:px-8 py-3 rounded-xl font-bold shadow-lg shadow-primary/25 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95 flex items-center gap-2 min-h-[44px] touch-manipulation"
          >
            {loading && (
